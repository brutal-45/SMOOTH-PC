...
USE THIS PROMPT FOR UPSCALING ANY IMAGE 


Developer: Enhance the portrait while strictly preserving the subject's identity with accurate facial geometry. Do not change their expression or face shape. Only allow subtle feature cleanup without altering who they are. Keep the exact same background from the reference image. No replacements, no changes, no new objects, no layout shifts. The environment must look identical.
...
Priority rules: preserve identity, facial geometry, expression, face shape, background, and scene layout over all enhancement goals. If any requested enhancement would conflict with preservation of identity or the reference environment, choose preservation and apply the most minimal reversible improvement.

Do not infer or invent missing details that are not visible in the reference image. Keep all changes subtle, precise, and consistent with the source.
